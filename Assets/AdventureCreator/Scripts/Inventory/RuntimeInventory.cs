...
namespace AC
{
	...
	public class RuntimeInventory : MonoBehaviour
	{
		...
		public void PerformCrafting (Recipe recipe, bool selectAfter)
		{
			...
			InvInstance addedInstance = playerInvCollection.AddToEnd (new InvInstance (recipe.resultID));
		...
		public void DrawSelectedInventoryCount ()
		{
			if (AC.KickStarter.cursorManager.displayCountSize <= 0)
			{
				return;
			}
			...
			if (InvInstance.IsValid (selectedInstance))
			{
				...
				if (displayCount > 1)
				{
					...
					{
						AdvGame.DrawTextEffect (AdvGame.GUIBox (cursorPosition, cursorSize), displayCount.ToString (), countStyle, Color.black, countStyle.normal.textColor, 2, countTextEffects);
					}
					else
					{
						GUI.Label (AdvGame.GUIBox (cursorPosition, cursorSize), displayCount.ToString (), countStyle);
					}
				}
			}
